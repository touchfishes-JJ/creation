...
public final class LockState {
    ...
    public static class Session { public final Mode mode; public final long end; public final String source; Session(Mode mode,long end,String source){this.mode=mode;this.end=end;this.source=source;} }
    ...
    public static Session current(Context c){
        ...
        if(m!=null&&now<end){try{return new Session(Mode.valueOf(m),end,"手动");}catch(Exception ignored){}} else if(m!=null)clearManual(c);
    ...
    public static Session weekendSession(Context c,long now){
        ...
        int minutes=cal.get(Calendar.HOUR_OF_DAY)*60+cal.get(Calendar.MINUTE);int[][] slots={{540,690},{810,1020},{1140,1290}};
        for(int idx=0;idx<slots.length;idx++){int[] s=slots[idx];if(minutes>=s[0]&&minutes<s[1]){Calendar e=(Calendar)cal.clone();e.set(Calendar.HOUR_OF_DAY,s[1]/60);e.set(Calendar.MINUTE,s[1]%60);e.set(Calendar.SECOND,0);e.set(Calendar.MILLISECOND,0);return new Session(weekendMode(c,idx),e.getTimeInMillis(),"周末");}}
        return null;
    }
    ...
    public static int currentWeekendSlot(){Calendar cal=Calendar.getInstance();int dow=cal.get(Calendar.DAY_OF_WEEK);if(dow!=Calendar.SATURDAY&&dow!=Calendar.SUNDAY)return -1;int minutes=cal.get(Calendar.HOUR_OF_DAY)*60+cal.get(Calendar.MINUTE);int[][] slots={{540,690},{810,1020},{1140,1290}};for(int i=0;i<slots.length;i++)if(minutes>=slots[i][0]&&minutes<slots[i][1])return i;return -1;}
    public static boolean slotFinished(int idx){Calendar cal=Calendar.getInstance();int dow=cal.get(Calendar.DAY_OF_WEEK);if(dow!=Calendar.SATURDAY&&dow!=Calendar.SUNDAY)return false;int minutes=cal.get(Calendar.HOUR_OF_DAY)*60+cal.get(Calendar.MINUTE);int[] end={690,1020,1290};return minutes>=end[idx];}
    ...
    public static String timeLeftShort(long end){long ms=Math.max(0,end-System.currentTimeMillis());long total=ms/1000,h=total/3600,m=(total%3600)/60,s=total%60;if(h>0)return String.format(Locale.CHINA,"%02d:%02d:%02d",h,m,s);return String.format(Locale.CHINA,"%02d:%02d",m,s);}
    public static String timeLeft(long end){long ms=Math.max(0,end-System.currentTimeMillis());long total=ms/1000,h=total/3600,m=(total%3600)/60,s=total%60;return String.format(Locale.CHINA,"%02d:%02d:%02d",h,m,s);}
}
