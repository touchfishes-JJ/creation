...
public final class LockState {
    ...
    private static final String[] SLOT_ENDS={"10:30","17:00","21:30"};
    ...
    public static void startManual(Context c, Mode mode, int minutes){
    ...
    public static Session weekendSession(Context c,long now){
        ...
        int[][] slots={{9*60,10*60+30},{13*60+30,17*60},{19*60,21*60+30}};
        for(int idx=0; idx<slots.length; idx++){
            ...
            if(minutes>=s[0] && minutes<s[1]){
                ...
                return new Session(weekendMode(c, idx),e.getTimeInMillis(),"周末");
            }
        }
        return null;
    }
    public static Mode weekendMode(Context c, int idx){
        String def = idx==0 ? Mode.EXAM.name() : (idx==1 ? Mode.AUDIO.name() : Mode.JOB.name());
        String raw = prefs(c).getString("weekend_slot_"+idx, def);
        try { return Mode.valueOf(raw); } catch (Exception e){ return Mode.EXAM; }
    }
    ...
    public static String slotStart(int idx){ return SLOT_STARTS[idx]; }
    public static String slotEnd(int idx){ return SLOT_ENDS[idx]; }
    public static int currentWeekendSlot(){
        Calendar cal=Calendar.getInstance();
        int dow=cal.get(Calendar.DAY_OF_WEEK);
        if(dow!=Calendar.SATURDAY && dow!=Calendar.SUNDAY) return -1;
        int minutes=cal.get(Calendar.HOUR_OF_DAY)*60+cal.get(Calendar.MINUTE);
        int[][] slots={{9*60,10*60+30},{13*60+30,17*60},{19*60,21*60+30}};
        for(int i=0;i<slots.length;i++) if(minutes>=slots[i][0] && minutes<slots[i][1]) return i;
        return -1;
    }
    public static boolean slotFinished(int idx){
        Calendar cal=Calendar.getInstance();
        int dow=cal.get(Calendar.DAY_OF_WEEK);
        if(dow!=Calendar.SATURDAY && dow!=Calendar.SUNDAY) return false;
        int minutes=cal.get(Calendar.HOUR_OF_DAY)*60+cal.get(Calendar.MINUTE);
        int[] end={10*60+30,17*60,21*60+30};
        return minutes>=end[idx];
    }
