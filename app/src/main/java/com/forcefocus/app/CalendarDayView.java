...
public class CalendarDayView extends View {
    ...
    @Override protected void onMeasure(int w,int h){int s=MeasureSpec.getSize(w); if(s<=0) s=dp(34); setMeasuredDimension(s,s);}    
    @Override protected void onDraw(Canvas c){
        super.onDraw(c);
        if(day<=0) return;
        float pad=dp(2), r=dp(9);
        RectF box=new RectF(pad,pad,getWidth()-pad,getHeight()-pad);
        p.setStyle(Paint.Style.FILL); p.setColor(Color.parseColor("#F4F0C7")); c.drawRoundRect(box,r,r,p);
        if(fraction>0){
            float top=box.bottom-box.height()*fraction;
            c.save();
            Path clip=new Path(); clip.addRoundRect(box,r,r,Path.Direction.CW); c.clipPath(clip);
            p.setColor(GREEN); c.drawRect(box.left,top,box.right,box.bottom,p);
            c.restore();
        }
    ...
    private void drawTree(Canvas c,float x,float y){
        ...
        p.setColor(Color.parseColor("#5A8D21"));
        c.drawCircle(x-dp(5),y+dp(6),dp(4.2f),p);
        c.drawCircle(x,y+dp(3),dp(4.8f),p);
        c.drawCircle(x+dp(5),y+dp(6),dp(4.2f),p);
        p.setColor(Color.parseColor("#7B4A1F"));
        c.drawRect(x-dp(1),y+dp(7),x+dp(1),y+dp(13),p);
    }
